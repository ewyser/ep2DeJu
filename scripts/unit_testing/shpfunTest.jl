...
@views function ϕ∂ϕCheck(ϕ∂ϕType)
    ...
    if ϕ∂ϕType == "bsmpm"
        ...
        p1=scatter!(xn,zeros(size(xn)),color="red",markersize=5,xlabel=L"$x$ [m]",ylabel=L"\phi_n(x_p)",markershape=:square,label="",show=true,aspect_ratio=1,c=:viridis,markerstrokecolor=:auto,markerstrokewidth=0,xlim=(xn[3]-dx/8,xn[end-2]+dx/8),ylim=(-0.1,1.25),colorbar_title="type",levels=5,title=L"$\log_{10}(\phi_n(x_p)) > -$"*string(abs(log10(tol))))
        ...
        p2=scatter!(xn,zeros(size(xn)),color="red",markersize=5,xlabel=L"$x$ [m]",ylabel=L"\partial_x\phi_n(x_p)",markershape=:square,label="",show=true,aspect_ratio=1,c=:viridis,markerstrokecolor=:auto,markerstrokewidth=0,xlim=(xn[3]-dx/8,xn[end-2]+dx/8),ylim=(-1,1),colorbar_title="type",levels=5,title=L"$\log_{10}(\phi_n(x_p)) > -$"*string(abs(log10(tol))))
        ...
        p3=scatter!(xn,zeros(size(xn)),color="red",markersize=5,xlabel=L"$x$ [m]",ylabel=L"$\sum_n\phi_n(x_p)$",markershape=:square,label="",show=true,aspect_ratio=1,c=:viridis,markerstrokecolor=:auto,markerstrokewidth=0,xlim=(xn[3]-dx/8,xn[end-2]+dx/8),ylim=(-0.1,1.5),colorbar_title="type",levels=5,title="partition of unity")
    else
    ...
    display(plot(p1,p2,p3; layout=(3,1), size=(550,650)))
